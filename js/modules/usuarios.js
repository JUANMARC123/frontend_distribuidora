...
async function save() {
    ...
    try {
        if (editingId) {
            ...
            const roles = [...document.querySelectorAll('input[name="roles"]:checked')].map(cb => parseInt(cb.value));
            ...
            const roles = [...document.querySelectorAll('input[name="roles"]:checked')].map(cb => parseInt(cb.value));
            if (newId && roles.length) await apiFetch(`/usuarios/${newId}/roles`, { method: 'POST', body: { roles } });
...
async function del(id, name) {
    const r = await Swal.fire({ title: '¿Eliminar usuario?', text: `Se eliminará "${name}".`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Sí, eliminar', cancelButtonText: 'Cancelar', confirmButtonColor: '#ef4444' });
    if (!r.isConfirmed) return;
    try { await apiFetch(`/usuarios/${id}`, { method: 'DELETE' }); Swal.fire('Eliminado', 'Usuario eliminado', 'success'); if (usuariosTable) await reloadDataTable(usuariosTable, '/usuarios'); }
